...
$mode = optional_param('mode', '', PARAM_NOTAGS);
...
// Set up the form.
$form = new uploadmetadata_form(null, array('fromdefault' => ''));
...
if (!$data) { // Display the form.
    ...
    // Display the form.
    $form->display();
	// Link to enable request for metadata update
	echo '<br/><a href="'.$url.'/index.php?mode=update">Update Metadata</a><br/>';
	// Process request for metadata update
	if ($mode=='update') {
		update_metadata();
	}
    ...
    $handler = new tool_uploadmetadata_handler($data->csvfile);
