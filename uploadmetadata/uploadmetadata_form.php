...
/**
 * Main form for uploading presentation metadata.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class uploadmetadata_form extends moodleform {
    ...
    public function definition() {
        ...
        // Heading.
        $mform->addElement('html', '<p>'.get_string('pluginname_help', 'tool_uploadmetadata').'</p>');

        // Insert a File picker element.
        $this->_form->addElement('filepicker', 'csvfile', get_string('file'));
        $this->_form->addHelpButton('csvfile', 'csvfile', 'tool_uploadmetadata');
    ...
    public function validation($data, $files) {
        ...
        if (empty($data['csvfile'])) {
            $errors['csvfile'] = get_string('uploadcsvfilerequired', 'tool_uploadmetadata');
        }
